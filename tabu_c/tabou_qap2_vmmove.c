...
float print_cost_sum(type_matrix a,          /* flows matrix */
                    ...
                    type_vector solution)
{
    ...
    for (i = 0; i < 1024; i++)
        for (j = 0; j < i; j++)
        {
            final_cost += a[i][j] * b[solution[i]][solution[j]];
            //printf("%f", final_cost);
        }
    printf("sum(distace * flow): %f\n", final_cost);
...
void transpose_vm(float *a, float *b)
{
    float temp = *a;
    *a = *b;
    *b = temp;
...

{
    ...
    *best_cost = current_cost;
    ...
    /* free memory*/
    free(p);
    for (i=0; i < n; i = i+1) free(delta[i]);
    free(delta);
...

{
    ...
    /**************** and matrix of cost of moves  *****************/
    current_cost = 0;
    for (i = 0; i < n; i = i + 1) for (j = 0; j < i; j = j + 1)
            current_cost = current_cost + a[i][j] * b[p[i]][p[j]];
    ...
    *best_cost = current_cost;
            ...
            current_iteration = current_iteration + 1)
    {
        ...
        else
        {
            ...
            if (current_cost < *best_cost)
            {
                printf("current_cost:%f vs best_cost:%d\n", current_cost, *best_cost);
                *best_cost = current_cost;
                for (k = 0; k < n; k = k+1) best_sol[k] = p[k];
...
//read data
// empty vm and pm
void init_bestfit_solution(int n, type_vector p, int input_mode)
{
    FILE* data_file;
    char file_name[100];
    int i;
    ...
    if(input_mode == 0){
        strcpy(file_name, "F:\\git\\workspace\\140512_vm_sort_original\\1_MC_BT_result\\nodes_result.data");
    }else{
        strcpy(file_name, "test\\mapping_result\\solution.data");
    }
    ...
    if (data_file != NULL)
    {
        for (i = 0; i < n; i = i+1)
        {
            fscanf(data_file,"%d", &p[i]);
            //printf("%d\n", p[i]);
        }
        printf("Read Nodes Result OK\n");
        fclose(data_file);
    }
    else
    {
...
int main(int argc, char ** argv)
{
    ...
    // scanf("%s",file_name);
    if(input_mode == 0){
        strcpy(file_name,"F:\\git\\workspace\\140512_vm_sort_original\\input\\vm_flow_matrix\\1Partitions@5percent.data");
    }else{
...
//// scanf("%s",file_name);
    //data_file = fopen(file_name,"r");
    strcpy(file_name,"input\\pm_distance\\pm_distanc_1024.data");
    data_file = fopen(file_name,"r");
    printf("Open file %s ...\n",file_name);
    if (data_file != NULL)
    {
    ...
    //scanf("%s",file_name);
    if(input_mode == 0){
        strcpy(file_name,"F:\\git\\workspace\\140512_vm_sort_original\\input\\vm_cost\\Node1024_cpu0.2_men0.2_stdvar1");
    }else{
    ...
    for(no_res = 1; no_res <= nr_resolutions; no_res++)
    {
        ...
        init_bestfit_solution(n, solution, input_mode);
        //print_cost_sum(c, b, m, solution);
        ...
        printf("\n %d Solution found by tabu search:\n", cost);
        for (i = 0; i < n; i = i+1) printf("%d ", solution[i]);

        printf("\n");
        somme_sol += cost;

        //final_result = print_cost_sum(c, b, m, solution);
    ...
    if(input_mode == 0){
        ...
        data_file = fopen("F:\\git\\workspace\\140512_vm_sort_original\\test\\test.txt", "at+");
        fprintf(data_file, "Average cost: %f, Solution result: %f\n",
          somme_sol/nr_resolutions, final_result);
    }else{
        data_file = fopen(argv[3], "at+");
        fprintf(data_file, "tabu_search: ");
        fprintf(data_file, "Average cost: %f, Solution result: %f\n",
        somme_sol/nr_resolutions, final_result);
    ...
    {
        //fscanf(data_file,"%d ", &p[i]);
        fprintf(data_file,"%d ", solution[i]);
    }
